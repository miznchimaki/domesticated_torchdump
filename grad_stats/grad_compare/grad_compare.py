...
from torchdump.utils import remove_path, write_csv, get_logger
from .grad_compare_utils import (
    check_path_exists,
    read_csv,
    create_directory,
    plt_savefig,
...
def readArgs():
    parser = argparse.ArgumentParser(description="configure parameters for grad_stats task")
    parser.add_argument(
        "--dump-dir1",
        type=str,
        default="./dump_dir",
        help="device1 grad_stats files",
    )
    parser.add_argument(
        "--dump-dir2",
        type=str,
        default="./dump_dir",
        help="device2 grad_stats files",
    )
    parser.add_argument(
        "--results-dir",
        type=str,
...
class GradComparator:
    ...
    @classmethod
    def compare_distributed(cls, path1, path2, output_dir):
    ...
    @classmethod
    def compare(cls, path1, path2, output_dir):
    ...
    @classmethod
    def _get_matched_dirs(cls, path1, path2, dir_prefix):
        check_path_exists(path1)
        check_path_exists(path2)
        dirs = []
    ...
    @classmethod
    def _save_similarities(cls, similarities, steps, output_dir):
        if not similarities:
    ...
    @classmethod
    def _get_matched_grad_files(cls, path1, path2, step):
        ...
        for grad_file in os.listdir(path1):
            ...
            if len(splits) < 1 or splits[-1] not in ["npy", "pt"]:
                continue
    ...
    @classmethod
    def _calculate_similarity(cls, grad_file1, grad_file2):
    ...
    @classmethod
    def _load_grad_files(cls, grad_file1, grad_file2):
...
def main():
    args = readArgs()
    dump_path1 = args.dump_dir1
    dump_path2 = args.dump_dir2
    results_path = args.results_dir
    GradComparator.compare_distributed(dump_path1, dump_path2, results_path)
